...
use crate::{map::DbMap, union::DbUnion, DbResult, TypeDb};
...
impl TypeDb {
    pub fn get_or_insert_crate_from_unions(&self, unions: Vec<DbUnion>) -> DbResult<DbCrate> {
        ...
        match db_crate {
            ...
            None => {
                let db_crate: DbResult<DbCrate> =
                    self.insert_crate_from_unions_and_lifts(unions, lifts);
                db_crate
            }
        }
    }
    ...
    fn get_crate_from_unions_and_lifts(
        ...
        crates.retain(|db_crate| db_crate.unions == unions && db_crate.lifts == lifts);
        // panic if there is more than one crate
        assert!(
            crates.len() <= 1,
            "critical error: more than one crate found: {:?}",
            crates
        );
    ...
    fn get_all_crates(&self) -> DbResult<Vec<DbCrate>> {
        ...
        let crates: DbResult<Vec<_>> = statement
            .query_map([], |row| {
    ...
    fn get_crate_from_uuid(&self, uuid: String) -> DbResult<DbCrate> {
        ...
        let unions: DbResult<Vec<DbUnion>> = unions
            ...
            .map(|(i, row)| {
                ...
                assert_eq!(
                    pos, i,
                    "critical error: union positions are not consecutive"
                );
                Ok(db_union)
            })
            .collect();
        ...
        let lifts = statement.query_map([&uuid], |row| {
            ...
            let lift: DbResult<DbLiftGivenUnions> = self
                .get_lift_from_uuid(lift_uuid)
                .transpose()
                .expect("lift not found");
            lift.map(|lift| (pos, lift))
        })?;
        let lifts: DbResult<Vec<DbLiftGivenUnions>> = lifts
            .enumerate()
            .map(|(i, row)| {
                let (pos, lift) = row?;
    ...
    fn get_lift_from_uuid(&self, uuid: String) -> DbResult<Option<DbLiftGivenUnions>> {
        ...
        let map: DbResult<Option<DbMap>> = statement
            .query_row([&uuid], |row| {
        ...
        let positions: Vec<(Option<usize>, Option<usize>)> = positions
            .into_iter()
            .enumerate()
            .map(|(i, row)| {
                ...
                assert_eq!(
                    pos, i,
                    "critical error: lift map positions are not consecutive"
                );
                Ok((in_pos, out_pos))
            })
            .collect::<DbResult<_>>()?;
    ...
    fn insert_crate_from_unions_and_lifts(
        ...
        db_crate
            ...
            .iter()
            .enumerate()
            .try_for_each(|(i, db_union)| {
                let _: usize =
                    statement.execute(rusqlite::params![&db_crate.uuid, i, &db_union.uuid])?;
                let res: DbResult<()> = Ok(());
                res
            })?;
        // insert lifts
        ...
        db_crate
            ...
            .iter()
            .enumerate()
            .try_for_each(|(i, db_lift)| {
                let _: usize =
                    statement.execute(rusqlite::params![&db_crate.uuid, i, &db_lift.uuid])?;
                let res: DbResult<()> = Ok(());
                res
            })?;
        // return the crate
    ...
    fn get_maps_from_unions(&self, unions: &[DbUnion]) -> DbResult<Vec<DbLiftGivenUnions>> {
        ...
        maps.retain(|map| map.in_outs.len() == unions.len());
        // finally, we filter maps by the in_outs
        let maps: DbResult<Vec<DbLiftGivenUnions>> = maps
            .into_iter()
            .filter_map(|map| {
                ...
                let in_out_union_pairs = map.in_outs.iter().zip(unions.iter());
                let positions: Option<Vec<(Option<usize>, Option<usize>)>> = in_out_union_pairs
                    .map(|(in_out, u)| {
                        ...
                        let in_position: Option<usize> = match in_field {
                            Some(in_field) => {
                                Some(u.fields.iter().position(|field| field == in_field)?)
                            }
                            None => None,
                        };
                        let out_position: Option<usize> = match out_field {
                            Some(out_field) => {
                                Some(u.fields.iter().position(|field| field == out_field)?)
                            }
                            None => None,
                        };
                    ...
                    .collect();
                let lift: Option<DbResult<DbLiftGivenUnions>> = positions
                    .map(|positions| self.get_or_insert_lift_given_unions(map, positions, unions));
                lift
            })
    ...
    fn get_or_insert_lift_given_unions(
        ...
        let lift: Option<DbLiftGivenUnions> =
            self.get_lift_given_unions(&map, &positions, unions)?;
        match lift {
            Some(lift) => Ok(lift),
            None => {
                // call the insert fn
                let lift: DbResult<DbLiftGivenUnions> =
                    self.insert_lift_given_unions(map, positions, unions);
                lift
            }
        }
    }
    ...
    fn get_lift_given_unions(
        ...
        lifts.retain(|lift| lift.positions.len() == unions.len());
        ...
        lifts.retain(|lift| {
            lift.positions
                ...
                .all(|(lift_pos, pos)| lift_pos == pos)
        });
        // `lifts` either has 0 or 1 elements (else, critical error, we'll panic for now)
        assert!(
            lifts.len() <= 1,
            "Critical error: more than one lift found for map {:?}",
            &map
        );
    ...
    fn get_lifts_given_map(&self, map_uuid: &str) -> DbResult<Vec<DbLiftGivenUnions>> {
        let mut statement = self
            .conn
            .prepare("SELECT uuid from lifts WHERE map_uuid = ?")?;
        let rows: DbResult<Vec<_>> = statement
            .query_map([map_uuid], |row| {
                let uuid: String = row.get(0)?;
                let lift: DbResult<DbLiftGivenUnions> = self
                    .get_lift_from_uuid(uuid)
                    .transpose()
                    .expect("Critical error: lift uuid not found");
                lift
            })?
            .collect::<DbResult<Vec<_>>>();
        rows
    ...
    fn insert_lift_given_unions(
        ...
        assert_eq!(
            lift.positions.len(),
            unions.len(),
            "Critical error: lift positions and unions have different lengths"
        );
