...
const CREATE_LIFT_POS_TABLE: &str = "
...
const CREATE_LIFT_CRATE_TABLE: &str = "
...
const CREATE_CRATE_UNION_TABLE: &str = "
...
impl TypeDb {
    pub(crate) fn create_new_crate_tables(&self) -> DbResult<()> {
        ...
        let _: usize = self.conn.execute(CREATE_CRATE_TABLE, [])?;
        let _: usize = self.conn.execute(CREATE_LIFT_TABLE, [])?;
        let _: usize = self.conn.execute(CREATE_LIFT_POS_TABLE, [])?;
        let _: usize = self.conn.execute(CREATE_LIFT_CRATE_TABLE, [])?;
        let _: usize = self.conn.execute(CREATE_CRATE_UNION_TABLE, [])?;
        Ok(())
    }
